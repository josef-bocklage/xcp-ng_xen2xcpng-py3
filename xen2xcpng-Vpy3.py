...
class Xva(object):
    ...
    def save_as(self, filename=None, sparse=False, username=None, password=None,
        ...
        if server:
            ...
            import base64
            auth = base64.encodebytes(("%s:%s" % (username, password)).encode()).decode().strip()
        ...
        string = io.StringIO(output_xml.decode('utf-8'))

#        string = io.StringIO(output_xml)
        string.seek(0)
        ...
        for disk in self.disks:
            ...
            while True:
                ...
                if zeroes == chunksize and not force and sparse:
                    ...
                    string = io.StringIO(input_buffer.decode('utf-8', errors='ignore'))
                    #byte_stream = io.BytesIO(input_buffer)
                    #string = io.StringIO(input_buffer.decode('utf-8'))
...
if __name__ == "__main__":
    ...
    if options.config:
        params = {}
        exec(compile(open(options.config, "rb").read(), options.config, 'exec'),params)
        if "name" in params: machine.set_name( params['name'] )
        if "vcpus" in params: machine.set_vcpus( params['vcpus'] )
        if 'kernel' in params:
